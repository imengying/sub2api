...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	// 2. 【新增】Wait后二次检查余额/订阅
	if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), apiKey.User, apiKey, service.QuotaPlatform(c.Request.Context(), apiKey)); err != nil {
			...
			if err != nil {
				...
				if errors.As(err, &promptTooLongErr) {
					...
					if !fallbackUsed && fallbackGroupID != nil && *fallbackGroupID > 0 {
						...
						if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), fallbackAPIKey.User, fallbackAPIKey, service.PlatformFromAPIKey(fallbackAPIKey)); err != nil {
							status, code, message, retryAfter := billingErrorDetails(err)
							if retryAfter > 0 {
								c.Header("Retry-After", strconv.Itoa(retryAfter))
							}
